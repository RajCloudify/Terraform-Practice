...
backend "s3" {
    ...
    dynamodb_table = "terraform-state-lock-dynamo-db" # Name of DynamoDB table used for state locking, note: first run day-4.2-s3-dynamodb resource so that it will create the resource first then run day-4.1-state-remote-backend
